...
static int tonegen_set_hwparams(snd_pcm_t *handle, 
  ...
  if (err < 0) 
    {
    printf("Access type not available: %s", snd_strerror(err));
    return err;
    }
  ...
  if (err < 0) 
    {
    printf("Can't set mono playback: %s",  
      snd_strerror(err));
    return err;
    }
  rrate = RATE;
  ...
  if (rrate != RATE) 
    {
    printf("Warning: Rate not available (requested %iHz, get %iHz)\n", 
       RATE, err);
    }
  ...
  *period_size = size;
  ...
  if (err < 0) 
    {
    printf("Unable to set hwparams: %s\n", snd_strerror(err));
    return err;
    }
  return 0;
  }
...
BOOL tonegen_setup_sound (snd_pcm_t **handle, const char *device, 
  ...
  if ((err = snd_pcm_open (handle, device, SND_PCM_STREAM_PLAYBACK, 0)) < 0) 
    {
    printf("Can't open playback device %s: %s", device, 
      snd_strerror(err));
    ret = FALSE;  
    }
  ...
  if (ret && (err = tonegen_set_hwparams (*handle, hwparams, 
    ...
    {
    printf("Can't set hwparams: %s", snd_strerror(err));
    ret = FALSE;  
    }
  if (ret && (err = tonegen_set_swparams (*handle, swparams,  
      buffer_size, *period_size)) < 0) 
    {
    printf("Can't set swparams: %s", snd_strerror(err));
    ret = FALSE;
    }
  return ret;
  }
